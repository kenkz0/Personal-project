...
code = code.replace(/\$\("#cardType"\)\.textContent = ([^;]+);/g, 'callbacks.onCardChange({ type: $1, name: plot.name, description: `Đường biên gồm ${plot.ring.length - 1} đỉnh, đọc trực tiếp từ “${plot.sourceName}”.`, area: plot.area, carbon: Math.min(900, Math.max(30, Math.round(plot.area * 8 * density / 100))), plot });');
...
// Analysis Panel
code = code.replace(/function updateAnalysisPanel\(plot\) \{([\s\S]*?)\n  \}/, 'function updateAnalysisPanel(plot) { callbacks.onAnalysisUpdate(plot); }');

// Map Updated
code = code.replace(/function markMapUpdated\(\) \{([\s\S]*?)\n  \}/, 'function markMapUpdated() { const value = new Intl.DateTimeFormat("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }).format(new Date()); callbacks.onMapUpdated(value); }');
...
code = code.replace(/\$\("#clearDrawn"\)\.addEventListener\("click", \(\) => \{([\s\S]*?)\n  \}\);/g, '');
...
code = code.replace(/\$\("#referenceToggle"\)\.addEventListener\("change", \(event\) => \{([\s\S]*?)\n  \}\);/g, 'function toggleReference(visible) { referenceLayer.show = visible; scene.requestRender(); }');
code = code.replace(/\$\("#satelliteToggle"\)\.addEventListener\("change", \(event\) => \{([\s\S]*?)\n  \}\);/g, 'function toggleSatellite(visible) { if(spectralLayer) spectralLayer.show = visible; indexOverlaySource.show = visible; scene.requestRender(); }');
...
code = code.replace(/\$\("#toggleAll"\)\.addEventListener\("click", \(event\) => \{([\s\S]*?)\n  \}\);/g, '');
code = code.replace(/\$\("#densityRange"\)\.addEventListener\("input", \(event\) => \{([\s\S]*?)\n  \}\);/g, 'function setDensity(val) { density = val; clearTimeout(densityTimer); densityTimer = setTimeout(rebuildForest, 120); }');
code = code.replace(/\$\("#toggle2D"\)\.addEventListener\("click", \(event\) => \{([\s\S]*?)\n  \}\);/g, 'function toggle2D() { is3D = !is3D; if (is3D) scene.morphTo3D(1); else scene.morphTo2D(1); }');
...
console.log('Fixed syntax and properly exported.');
